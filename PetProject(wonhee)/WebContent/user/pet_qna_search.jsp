...
<body>
    ...
    <div id="sub_wrap" class="webfont">
        ...
        <div id="section">
            ...
            <p class="sub_title">질문과 답변 검색결과<span class="sub_title2">QnA</span></p>
                ...
                <div id="table_wrap">
                        ...
                        <tr>
                            <td colspan="4" align="right" class="table_btn">
                            	 <input type="button" value="목록으로"
                                       			onclick="location.href='<%=request.getContextPath() %>/pet_QA.do'">
                    ...
                    <div class="ext_paging">
						...
						<c:if test="${page > block }">
							<a href="pet_QA_search.do?page=1&search_field=${search_field }&search_keyword=${search_keyword}"> [맨처음] </a>
							<a href="pet_QA_search.do?page=${startBlock -1 }&search_field=${search_field }&search_keyword=${search_keyword}"> &lt;<span class="page_s">이전</span> | </a>&nbsp;
						</c:if>
						
						<c:forEach begin="${startBlock }" end="${endBlock }" var = "i">
							<c:if test="${i == page }">
								<b><a href = "pet_QA_search.do?page=${i }&search_field=${search_field }&search_keyword=${search_keyword}">[${i }]</a></b>
							</c:if>
							<c:if test="${i != page }">
								<a href = "pet_QA_search.do?page=${i }&search_field=${search_field }&search_keyword=${search_keyword}">[${i }]</a>
							</c:if>
						</c:forEach>
						
						<c:if test="${endBlock < allPage }">
							&nbsp;<a href="pet_QA_search.do?page=${endBlock + 1 }&search_field=${search_field }&search_keyword=${search_keyword}"> | <span class="page_s">다음</span>&gt; </a>
							<a href="pet_QA_search.do?page=${allPage }&search_field=${search_field }&search_keyword=${search_keyword}"> [마지막] </a>
						</c:if>
                    </div>
